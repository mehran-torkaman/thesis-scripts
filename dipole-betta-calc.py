...
charge = 0.417
for v in range(101):
    file_betta = 'dipole-betta'+str(v)
    data = np.loadtxt(str(v))
#========================================
...
#===============================================
    rad_betta = list()
    for i in range(1,num):
        betta = np.arccos(dipoles['molecule'+str(i)][1]/dipoles['molecule'+str(i)][3])
        rad_betta.append(betta)
    result_betta = list()
    for i in rad_betta:
        result_betta.append(np.degrees(i))
    degree_betta = dict()
    for i in range(1,num):
        degree_betta['molecule'+str(i)]=result_betta[i-1]
#=====================================
    ...
    for i in np.arange(min_h,max_h,tickness):
        for j in range(1,num):
            if obj['molecule'+str(j)]['O'][2] >= i and obj['molecule'+str(j)]['O'][2] < i+tickness:
                for k in range(0,181,dig_step):
                    if degree_betta['molecule'+str(j)] >= k and degree_betta['molecule'+str(j)] < k+dig_step:
                        z_chunks_betta[i][k].append(1)
#========================
# Outputs
#========================
    with open(file_betta, 'w') as f:
        for key,value in z_chunks_betta.items():
...
#========================
    if tickness == max_h:
        ...
        for i in range(0,181):
            bins.append(i)
        for k,value in z_chunks_betta.items():
            for k,v in value.items():
