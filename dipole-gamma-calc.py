...
#===============================================
    ...
    for i in range(1,num):
        ...
        rad_gamma.append(gamma)
    result_gamma = list()
    for i in rad_gamma:
        result_gamma.append(np.degrees(i))
    degree_gamma = dict()
    for i in range(1,num):
        degree_gamma['molecule'+str(i)]=result_gamma[i-1]
#=====================================
        ...
        for j in range(1,num):
            if obj['molecule'+str(j)]['O'][2] >= i and obj['molecule'+str(j)]['O'][2] < i+tickness:
                for k in range(0,181,dig_step):
                    if degree_gamma['molecule'+str(j)] >= k and degree_gamma['molecule'+str(j)] < k+dig_step:
                        z_chunks_gamma[i][k].append(1)
#========================
# Outputs
#========================
    with open(file_gamma, 'w') as f:
        for key,value in z_chunks_gamma.items():
...
#========================
    if tickness == max_h:
        ...
        for i in range(0,181):
            bins.append(i)
        for k,value in z_chunks_gamma.items():
            for k,v in value.items():
