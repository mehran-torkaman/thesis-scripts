...
#===============================================
    rad_alpha = list()
    for i in range(1,num):
        alpha = np.arccos(dipoles['molecule'+str(i)][0]/dipoles['molecule'+str(i)][3])
        rad_alpha.append(alpha)
    result_alpha = list()
    for i in rad_alpha:
        result_alpha.append(np.degrees(i))
    degree_alpha = dict()
    for i in range(1,num):
        degree_alpha['molecule'+str(i)]=result_alpha[i-1]
#=====================================
        ...
        for j in range(1,num):
            if obj['molecule'+str(j)]['O'][2] >= i and obj['molecule'+str(j)]['O'][2] < i+tickness:
                for k in range(0,181,dig_step):
                    if degree_alpha['molecule'+str(j)] >= k and degree_alpha['molecule'+str(j)] < k+dig_step:
                        z_chunks_alpha[i][k].append(1)
#========================
# Outputs
#========================
    with open(file_alpha, 'w') as f:
        for key,value in z_chunks_alpha.items():
...
#========================
    if tickness == max_h:
        ...
        for i in range(0,181):
            bins.append(i)
        for k,value in z_chunks_alpha.items():
            for k,v in value.items():
